...
rawNetData$timestamp <- as.POSIXct(strptime(rawNetData$posted_at, '%a %b %d %H:%M:%S %z %Y', tz = "GMT"), tz = "GMT")
node_att <- read.csv(file = "node_CLN.csv", header = TRUE, as.is = TRUE, stringsAsFactors = FALSE, encoding = "utf-8")
node_att_CLN <- node_att %>% distinct(site, .keep_all = TRUE)
rawNetData <- left_join(rawNetData, node_att_CLN)
...
antivax_name <- c("New York Post","Daily Caller","Fox News","One America News Network",
                  "Washington Examiner","The Blaze")
gov_name <- c("CDC","FDA","WHO","White House")
provax_name <- main_name[!main_name$expand_name %in% antivax_name &
                           !main_name$expand_name %in% gov_name,]
...
fake_name <- NetData %>% 
  ...
  filter(expand_name != "Global Times"
            ...
            & expand_name != "IFL Science"
            & expand_name != "Autism Speaks"
            & expand_name != "Democratic National Committee") %>%
  filter(!is.na(expand_name)) %>%
  distinct(expand_name)
...
d=data.frame(date=as.Date(c("2021-01-26", "2021-03-13", "2021-04-21", "2021-05-10", "2021-06-01")), 
             event=c("More than 23 million COVID-19 vaccine doses have been administered in the U.S.", 
                     ...
                     "FDA expands the emergency use for teenagers", 
                     "Delta variant became dominant variant in the U.S."))
...
a <- ggplot(data = dd_share_fake) + 
  ...
  #geom_text(data=d, mapping=aes(x=date, y=0, label=event), size=4, angle=0, vjust=-0.4, hjust=0) +
  xlab("") +
  theme_classic() +
...
sample <- NetData_core_bp_upd[expand_group == "fake news",] ### 323613 tweets (analytic)
length(unique(sample$site_ID)) #unique publishers: 310
length(unique(sample$user_ID)) #unique users: 103110
...
for (i in 1:168) {
  ...
  test <- NetData_core_bp_upd[date == date_id &
                              ...
                              c("site_ID","user_ID")]  %>% distinct()
  test2 <- NetData_core_bp_upd[date == date_id &
                                #date <= date_id_end &
                                expand_name != user_name &
                                user_name != "GMWatch" &
                                expand_group == "trusted media",
                              c("site_ID","user_ID")] %>% distinct()
  # attachment to mainstream media/government
  # number of users who also shared info from mainstream media/government
  antivax_id <- c(unique(NetData_core_bp_upd[NetData_core_bp_upd$expand_name %in% antivax_name,c("site_ID")]))
  provax_id <- c(unique(NetData_core_bp_upd[NetData_core_bp_upd$expand_name %in% provax_name,c("site_ID")]))
  gov_id <- c(unique(NetData_core_bp_upd[NetData_core_bp_upd$expand_name %in% gov_name,c("site_ID")]))
  
  main.list[[i]] <- test %>%
    group_by(site_ID) %>% 
    dplyr::summarize(users = n(),
                     engage_site = length(unique(test2$site_ID[test2$user_ID %in% user_ID])),
                     antivax_engage_site = length(unique(test2[test2$site_ID %in% antivax_id$site_ID,]$site_ID[test2[test2$site_ID %in% antivax_id$site_ID,]$user_ID %in% user_ID])),
                     provax_engage_site = length(unique(test2[test2$site_ID %in% provax_id$site_ID,]$site_ID[test2[test2$site_ID %in% provax_id$site_ID,]$user_ID %in% user_ID])),
                     gov_engage_site = length(unique(test2[test2$site_ID %in% gov_id$site_ID,]$site_ID[test2[test2$site_ID %in% gov_id$site_ID,]$user_ID %in% user_ID])))

  #engage_user = sum(user_ID %in% test2$user_ID),
  #antivax_engage_user = sum(user_ID %in% test2[test2$site_ID %in% antivax_id$site_ID,]$user_ID),
  #provax_engage_user = sum(user_ID %in% test2[test2$site_ID %in% provax_id$site_ID,]$user_ID),
  #gov_engage_user = sum(user_ID %in% test2[test2$site_ID %in% gov_id$site_ID,]$user_ID)
...
for (i in 1:168) {
  ...
  temp_f2 <- left_join(temp_f, ego_contagion, by=c("site_ID"))
  ego_engage <- main.list[[i]]
  ego_engage$site_ID <- as.character(ego_engage$site_ID)  
  temp <- left_join(temp_f2, ego_engage, by=c("site_ID"))
...
cols = c("aud","deg","engage_site","antivax_engage_site","provax_engage_site","gov_engage_site",
         "isoego","trans_local","logmeanfw","trans_localsq","aud.cen","propsen")
...
site_meta <- read.csv("site_metadata_03022023.csv")
...
message.panel.ana <- message.panel[time_id>1,c("site_ID","user_ID",
                                               ...
                                               "logfollower",
                                               "date","time_id",
                                               "lag_aud","lag_deg","lag_engage_site","lag_antivax_engage_site",
                                               "lag_provax_engage_site","lag_gov_engage_site","lag_isoego","lag_trans_local",
                                               ...
                                               "year","traffic","Media.Type","type")]
message.panel.ana$cgn <- message.panel.ana$retweet_count + message.panel.ana$favorite_count

##descriptive statistics
library(vtable)
st(site.panel.ana[!is.na(site.panel.ana$lag_isoego),], out="csv", file = "descriptive_site.csv")
st(message.panel.ana[!is.na(message.panel.ana$lag_isoego),], out="csv", file = "descriptive_message.csv")
write.csv(site.panel.ana, "sitepanel.csv")
write.csv(message.panel.ana, "messagepanel.csv")
...
library(tidyverse)
library(MASS)
data <- read_dta("messagepanel_R.dta")
m7 <- fenegbin(cgn ~ lag_trans_local + 
                 lag_trans_localsq + lag_engage_site + logfollower +
                 sen  + as.factor(country) + as.factor(rev) + as.factor(ntraf) + as.factor(ntype)
               |  as.factor(user_id2), data = data, nthreads = 8, vcov = 'hc1')
summary(m7)
m8 <- fenegbin(cgn ~ lag_trans_local + 
                 lag_trans_localsq + lag_antivax_engage_site + lag_provax_engage_site + lag_gov_engage_site + 
                 logfollower +
                 sen  + as.factor(country) + as.factor(rev) + as.factor(ntraf) + as.factor(ntype)
               |  as.factor(user_id2), data = data, nthreads = 8, vcov = 'hc1')
summary(m8)
etable(m7,m8, digits=3, file ="tableA3c.rtf", tex = TRUE)